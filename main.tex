...
\begin{enumerate}
  \item Give a more concrete representation of $H^{1}(X,\mathcal{L}(D))$.
  \item Construct a perfect pairing between $H^{1}(X,\mathcal{L}(D))$
        and some specific vector space of differentials.
  \item Show that this vector space is isomorphic to
        $H^{0}(X,K_{X}\otimes\mathcal{L}(D)^{\vee})$.
\end{enumerate}
