...
\subsection{Defining sheaves}\label{subsec:sheaf_defs}
...
\begin{enumerate}
  \item Any fibre bundle $\pi\colon E\to B$ defines a sheaf by
        \[
        \mathscr{E}(U) = \Set{s:U\to E\mid s\text{ a section of }\pi}.
        \]
  \item Every affine variety $X$ carries the structure sheaf $\mathscr{O}_{X}$ defined by
        \[
        \mathscr{O}_{X}(U)=\Set{f\in k(X)\mid f\text{ regular on } U}.
        \]
  \item Let $A$ be a group and $P\in X$ a point. The skyscraper sheaf $A_{P}$
        is defined by
        \[
        A_{P}(U)=\begin{cases}
          A, & \quad P\in U \\
        ...
        \end{cases}
        \]
  \item Suppose $X$ is any topological space and $V\subseteq X$ is an open subset.
        If $\mathscr{F}$ is a sheaf on $X$, then we can restrict it to
        a sheaf $\mathscr{F}\vert_{V}$ on $V$ by defining
        \[
        \mathscr{F}\vert_{V}(U)=\mathscr{F}(U\cap V).
        \]
  \item If $f\colon X\to Y$ is a continuous map of topological spaces and
        $\mathscr{F}$ is a sheaf on $X$, then we can construct the so-called
        pushforward sheaf $f_{\ast}\mathscr{F}$ on $Y$ by defining
        \[
        f_{\ast}\mathscr{F}(U)=\mathscr{F}(f^{-1}(U)).
        \]
\end{enumerate}
\begin{bcat}% TODO: Cite
  Let $X$ be a topological space and $\iota\colon V\hookrightarrow X$ an open subset.
  The assignments
  \[
    \iota_{\ast}\colon\textbf{Sh}(V)\to\textbf{Sh}(X)\quad\text{and}\quad
    (-)\vert_{V}\colon\textbf{Sh}(X)\to\textbf{Sh}(V)
    :\mathscr{F}\mapsto\mathscr{F}\vert_{V}
  \]
  are functorial and define an adjoint pair $\iota_{\ast}\dashv(-)_{V}$.

  More generally, one can define the \emph{pullback} $f^{\ast}\mathscr{F}$
  of a sheaf $\mathscr{F}$ for any continuous map $f\colon X\to Y$,
  which is the left adjoint to the pushforward $f_{\ast}$.
\end{bcat}
...
\begin{defin}[{\cite[Def. 2.3.15]{gathmann}}]
...
space.
\begin{defin}[{\cite[{Def. 2.4.1}]{gathmann}}]
...
\begin{defin}[{\cite[{Def. 2.5.1}]{gathmann}}]
...
\begin{lemm}[{\cite[{Prop. 10.1.8}]{vakil}}]%
...
\begin{thm}[{\cite[{Lemma 13.21}]{gathmann_new}}]%
...
Indeed, most sheaves that algebraic geometers are interested in belong to this
class. The definition is algebraic and makes more sense in the framework
of scheme theory, but we can state it in elementary terms now. The reason
we introduce quasi-coherent sheaves is that Lemma~\ref{lemm:qcoh_local_global}
below is what makes our definition of \v Cech cohomology work
(Subsection~\ref{subsec:cech_cohomology}).
\begin{defin}
  Suppose $X$ is an affine variety and $M$ is a module over the coordinate
  ring $\mathscr{O}_{X}(X)$. Then, the sheaf $\cm$ associated to the
...
\end{rem}
\begin{rem}
  The constructions in Def.~\ref{def:sheaf_algebra} preserve quasi-coherence.
  I refer the reader to \cite{gathmann}.
\end{rem}
\begin{ex}
  The structure sheaf $\mathscr{O}_{X}$ of a variety $X$ is always
  quasi-coherent with $\mathscr{O}_{X}=\cm$, where
  $M=\mathscr{O}_{X}(X)$.
\end{ex}
...
\begin{prop}[{\cite[Lemma 7.2.7]{gathmann}}]\label{prop:qcoh_gsec_exact}
