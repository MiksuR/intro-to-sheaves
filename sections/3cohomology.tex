...
\begin{cat}
  ...
  k[X]$\textbf{-Mod} is left-exact but not right-exact.
\end{cat}
...
\textbf{the} definition of sheaf cohomology. The primary source in the
next subsection is \cite{vakil}.
...
\begin{defin}
  ...
  \begin{enumerate}
    \item it is additive: if $A$, $B$ are objects of $\mathcal{A}$
          and $f, g\in\text{Hom}(A, B)$, then $F(f+g)=F(f)+F(g)$.
    \item given a SES
          \[\begin{tikzcd}
...
\begin{proof}
  ...
  \begin{equation}\label{comp:prod_resolution}
    ...
    \end{tikzcd},
  \end{equation}
  since additive functors preserve finite products. Since $A_{1}$ and $A_{2}$
...
\begin{proof}
  ...
  in \cite{hartshorne} (they are flasque and flasque sheaves are acyclic).
  Therefore, the sequence $0\to \cm\to\widetilde{I}^{\bullet}$ is an
  acyclic resolution of $\cm=\mathscr{F}$. Applying the global sections
  ...
  groups at $I^{i}$ are still zero for $i>0$.
\end{proof}
...
\[\begin{tikzcd}
  ...
  \end{tikzcd},\]
which looks awfully like a beginning of an acyclic resolution in light of
Lemma~\ref{lemm:affine_cohom_vanishes}. Thus, I would like to find sheaves
with global sections that match the ones in the exact sequence. But this is
quite simple in fact; I simply take the sheaves
...
\begin{proof}
  ...
  {\renewcommand{\qedsymbol}{$\blacksquare$}
    \begin{proof}
      ...
      $k[U]$-module $N$. Note that since the sheaf $\mathscr{I}^{i}$ is a
      quasi-coherent sheaf, there is a $k[V\cap U]$-module $M$ such that
      ...
      $k[U]\to k[V\cap U]$. We can the define $N$ as the restriction of
      scalars of $M$ along this homomorphism, and then
      $\mathscr{I}_{+}^{i}\vert_{U}=\widetilde{N}$.
...
\begin{defin}\label{def:cech}
  The \v Cech cohomology of a quasi-coherent sheaf $\mathscr{F}$ the
