...
\end{prop}
Note, $f_{P}\in k(X)$ and $\mathscr{O}_{X,P}$ is a space of germs. Thus,
when I write $f_{P}\in\mathscr{O}_{X,P}$, I actually mean $(f_{P})_{P}
\in\mathscr{O}_{X,P}$. I will continue using the simpler notation through out
the paper.
\begin{proof}
  % TODO: This proof doesn't need to be this complicated since X
  %       is assumed to be irreducible -> replace it by a shorter argument.
  Fix an arbitrary $P\in X$. Since $\underline{k(X)}$ is obtained by
  sheafification of the constant presheaf, there must be an open
  neighbourhood $U\subseteq X$ of $P$ and $g\in k(X)$ such that
...
\begin{rem}
  ...
  \[
    ...
    k(X)/\mathcal{L}(D)_{P},
  \]
  which might be a helpful way of thinking this space of global sections.
  % TODO: Justify this remark
\end{rem}
...
family a \emph{r\'epartition} in \cite{serre}).
...
Now I can proceed to prove the bijectivity of $\iota_{D}$. The proof relies
